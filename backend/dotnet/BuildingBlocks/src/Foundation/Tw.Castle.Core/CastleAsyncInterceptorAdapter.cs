...
public sealed class CastleAsyncInterceptorAdapter : CastleAsyncInterceptor, CastleInterceptor
{
    ...
    /// <summary>
    /// 保存当前类型处理流程依赖的InterceptAsynchronous使用结果Method
    /// </summary>
    ...
    /// <summary>
    /// 保存当前类型处理流程依赖的interceptionReport
    /// </summary>
        ...
        IServiceProvider serviceProvider)
    {
        ...
        _interceptionReport = serviceProvider.GetService(typeof(InterceptionReport)) as InterceptionReport;
    }
    ...
    /// <summary>
    /// 说明解析MostSpecific服务Interface在当前类型中的职责
    /// </summary>
    /// <param name="invocation">用于提供nvocation</param>
    /// <param name="implementationType">服务注册中使用的实现类型</param>
    /// <param name="declaredServiceType">用于提供declared服务类型</param>
    /// <param name="method">用于构造测试场景的方法元数据</param>
    ...
    private IReadOnlyList<AbstractionInterceptor> ResolveInterceptors(CastleInvocation invocation)
    {
        ...
        {
            var interceptor = _serviceProvider.GetService(interceptorType)
    ...
    /// <summary>
    /// 说明nvokePipelineAsTaskAsync在当前类型中的职责
    /// </summary>
    /// <param name="invocation">用于提供nvocation</param>
