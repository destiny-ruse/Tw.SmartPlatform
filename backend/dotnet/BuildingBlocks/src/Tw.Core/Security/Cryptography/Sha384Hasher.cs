...
/// <summary>
/// Provides SHA-384 hash computation and verification helpers.
/// </summary>
public static class Sha384Hasher
{
    ...
    /// <returns>The SHA-384 hash as a hexadecimal string.</returns>
    public static string ComputeHash(string input, bool useUpperCase = false, Encoding? encoding = null)
    {
        return HashComputation.ComputeHash(input, useUpperCase, encoding, SHA384.HashData);
    }

    /// <summary>Computes the SHA-384 hash for bytes.</summary>
    /// <param name="bytes">The bytes to hash.</param>
    /// <param name="useUpperCase">Whether to return uppercase hexadecimal characters.</param>
    /// <returns>The SHA-384 hash as a hexadecimal string.</returns>
    public static string ComputeHash(byte[] bytes, bool useUpperCase = false)
    {
        return HashComputation.ComputeHash(bytes, useUpperCase, SHA384.HashData);
    }

    /// <summary>Computes the SHA-384 hash for a file.</summary>
    ...
    {
        return HashComputation.ComputeFileHashAsync(filePath, useUpperCase, SHA384.HashDataAsync, cancellationToken);
    }

    /// <summary>Computes the SHA-384 hash for a stream without disposing it.</summary>
    ...
    {
        return HashComputation.ComputeFileHashAsync(stream, useUpperCase, SHA384.HashDataAsync, cancellationToken);
    }

    /// <summary>Verifies a SHA-384 hash for a string using fixed-time byte comparison.</summary>
    ...
    {
        return HashComputation.VerifyHash(input, hash, encoding, SHA384.HashData);
    }

    /// <summary>Verifies a SHA-384 hash for bytes using fixed-time byte comparison.</summary>
    /// <param name="bytes">The bytes to hash and verify.</param>
    /// <param name="hash">The expected hexadecimal hash.</param>
    /// <returns><see langword="true"/> when the hash matches; otherwise, <see langword="false"/>.</returns>
    public static bool VerifyHash(byte[] bytes, string hash)
    {
        return HashComputation.VerifyHash(bytes, hash, SHA384.HashData);
    }
}
