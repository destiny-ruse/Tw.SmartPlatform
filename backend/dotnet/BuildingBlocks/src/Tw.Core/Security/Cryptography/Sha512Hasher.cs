...
/// <summary>
/// Provides SHA-512 hash computation and verification helpers.
/// </summary>
public static class Sha512Hasher
{
    ...
    {
        return HashComputation.ComputeHash(input, useUpperCase, encoding, SHA512.HashData);
    }

    /// <summary>Computes the SHA-512 hash for bytes.</summary>
    /// <param name="bytes">The bytes to hash.</param>
    /// <param name="useUpperCase">Whether to return uppercase hexadecimal characters.</param>
    /// <returns>The SHA-512 hash as a hexadecimal string.</returns>
    public static string ComputeHash(byte[] bytes, bool useUpperCase = false)
    {
        return HashComputation.ComputeHash(bytes, useUpperCase, SHA512.HashData);
    }

    /// <summary>Computes the SHA-512 hash for a file.</summary>
    ...
    {
        return HashComputation.ComputeFileHashAsync(filePath, useUpperCase, SHA512.HashDataAsync, cancellationToken);
    }

    /// <summary>Computes the SHA-512 hash for a stream without disposing it.</summary>
    ...
    {
        return HashComputation.ComputeFileHashAsync(stream, useUpperCase, SHA512.HashDataAsync, cancellationToken);
    }

    /// <summary>Verifies a SHA-512 hash for a string using fixed-time byte comparison.</summary>
    ...
    {
        return HashComputation.VerifyHash(input, hash, encoding, SHA512.HashData);
    }

    /// <summary>Verifies a SHA-512 hash for bytes using fixed-time byte comparison.</summary>
    /// <param name="bytes">The bytes to hash and verify.</param>
    /// <param name="hash">The expected hexadecimal hash.</param>
    /// <returns><see langword="true"/> when the hash matches; otherwise, <see langword="false"/>.</returns>
    public static bool VerifyHash(byte[] bytes, string hash)
    {
        return HashComputation.VerifyHash(bytes, hash, SHA512.HashData);
    }
}
