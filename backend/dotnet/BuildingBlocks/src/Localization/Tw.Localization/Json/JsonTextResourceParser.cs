...
public static class JsonTextResourceParser
{
    ...
    /// <exception cref="LocalizationConfigurationException">当 JSON 格式无效或不符合规范时抛出</exception>
    public static JsonTextResource Parse(string resourceName, string sourcePath, string json)
    {
        ...
        try
        {
            ...
            {
                throw new LocalizationConfigurationException($"JSON 多语言资源根节点必须是对象：{validatedSourcePath}");
            }

            if (!root.TryGetProperty("culture", out var cultureElement) ||
                cultureElement.ValueKind != JsonValueKind.String)
            {
                throw new LocalizationConfigurationException($"JSON 多语言资源缺少 culture 字符串：{validatedSourcePath}");
            }
            ...
            {
                throw new LocalizationConfigurationException($"JSON 多语言资源 culture 无效：{validatedSourcePath}");
            }

            if (!root.TryGetProperty("texts", out var textsElement) ||
                textsElement.ValueKind != JsonValueKind.Object)
            {
                throw new LocalizationConfigurationException($"JSON 多语言资源缺少 texts 对象：{validatedSourcePath}");
            }
        ...
        {
            throw new LocalizationConfigurationException($"JSON 多语言资源格式错误：{validatedSourcePath}", exception);
        }
    }
        ...
        string sourcePath)
    {
        foreach (var property in element.EnumerateObject())
        {
            ...
            throw new LocalizationConfigurationException($"JSON 多语言资源 texts 叶子值必须是字符串：{sourcePath}");
        }
    }
}
