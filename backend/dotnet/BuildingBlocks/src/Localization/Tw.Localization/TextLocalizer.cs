...
public sealed class TextLocalizer : ITextLocalizer
{
    ...
    /// 当 <paramref name="contributors"/> 或 <paramref name="options"/> 为 <see langword="null"/> 时抛出
    /// </exception>
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            var result = await _contributors[i].GetOrNullAsync(request, cancellationToken).ConfigureAwait(false);
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            await contributor.FillAsync(request, texts, cancellationToken).ConfigureAwait(false);
        }
