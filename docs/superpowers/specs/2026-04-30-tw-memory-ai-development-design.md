...
1. 基于 `docs-old/standards` 现有内容重写规范正文。
2. 去除旧的分片索引方式。
3. 规范正文面向人阅读和公司治理。
4. `.tw-memory` 只生成规范索引、chunk 元数据和证据指针。
5. 语言 skill 必须先按需读取规范。
6. 人工修改规范后运行 `generate/check` 更新 `.tw-memory`。
7. AI 修改规范正文时写入 `docs/standards`，再运行 `generate/check`。
