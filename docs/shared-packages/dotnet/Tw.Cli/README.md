...
`audit dependencies` 扫描 `PackageReference` 和 `ProjectReference`，检查以下边界：
...
`diagnose` 输出实际发现的 BuildingBlocks 运行时/测试项目数量、`.slnx` 一致性、未解析 `ProjectReference`、淘汰引用、缺失锁文件和锁文件内淘汰依赖。命令会执行以下权威检查，并原样传播非零退出码：
