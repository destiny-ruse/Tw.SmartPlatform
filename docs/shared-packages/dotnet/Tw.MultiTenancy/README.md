...
`Tw.MultiTenancy` 提供 provider-neutral 的当前租户契约、租户身份值对象，以及认证后令牌租户与调用方提示租户的一致性解析。包内不读取 HTTP 上下文，也不访问租户存储。
...
- 包保持 provider-neutral，不依赖 ASP.NET Core、SqlSugar、CAP 或其他租户 provider
- JWT 签名与声明校验由认证边界负责
- HTTP header、route 或 query accessor 由 Web 适配层负责
- 租户目录、租户配置和持久化存储由具体 provider 或业务服务负责
- 包不校验租户标识格式，也不提供分片策略
