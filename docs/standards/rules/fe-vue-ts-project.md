...
summary: 规定 Vue TypeScript 项目的目录、组件边界和构建配置。
machine_rules: []
supersedes: []
superseded_by:
...
Vue TypeScript 项目必须让页面、组件、组合式函数、请求层和构建配置形成清晰边界，避免功能目录互相穿透和组件职责膨胀。统一项目结构可以让新增页面、迁移模块和 AI 辅助修改都落在可评审的位置。
