...
def format_master_md(design_system: dict) -> str:
    ...
    if anti_patterns:
        ...
        for anti in anti_list:
            if anti:
                lines.append(f"- 鉂?{anti}")
    lines.append("")
    lines.append("### Additional Forbidden Patterns")
    lines.append("")
    lines.append("- 鉂?**Emojis as icons** 鈥?Use SVG icons (Heroicons, Lucide, Simple Icons)")
    lines.append("- 鉂?**Missing cursor:pointer** 鈥?All clickable elements must have cursor:pointer")
    lines.append("- 鉂?**Layout-shifting hovers** 鈥?Avoid scale transforms that shift layout")
    lines.append("- 鉂?**Low contrast text** 鈥?Maintain 4.5:1 minimum contrast ratio")
    lines.append("- 鉂?**Instant state changes** 鈥?Always use transitions (150-300ms)")
    lines.append("- 鉂?**Invisible focus states** 鈥?Focus states must be visible for a11y")
    lines.append("")
...
def format_page_override_md(design_system: dict, page_name: str, page_query: str = None) -> str:
    ...
    lines.append("")
    lines.append("> 鈿狅笍 **IMPORTANT:** Rules in this file **override** the Master file (`design-system/MASTER.md`).")
    ...
    if layout:
        ...
        lines.append("- No overrides 鈥?use Master layout")
    lines.append("")
    ...
    if spacing:
        ...
        lines.append("- No overrides 鈥?use Master spacing")
    lines.append("")
    ...
    if typography:
        ...
        lines.append("- No overrides 鈥?use Master typography")
    lines.append("")
    ...
    if colors:
        ...
        lines.append("- No overrides 鈥?use Master colors")
    lines.append("")
    ...
    if components:
        ...
        lines.append("- No overrides 鈥?use Master component specs")
    lines.append("")
...
def _generate_intelligent_overrides(page_name: str, page_query: str, design_system: dict) -> dict:
    ...
    if style_results:
        ...
        if any(kw in keywords.lower() for kw in ["data", "dense", "dashboard", "grid"]):
            ...
            spacing["Content Density"] = "High 鈥?optimize for information display"
            ...
            spacing["Content Density"] = "Low 鈥?focus on clarity"
        else:
